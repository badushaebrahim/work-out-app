      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="bg-surface-container-low rounded-2xl border border-white/5 p-6">
          <div className="flex items-center justify-between mb-8">
            <h2 className="font-headline font-bold text-xl uppercase tracking-widest text-white">Workouts</h2>
              ...
              workouts.map((workout: any) => (
                <div key={workout._id.toString()} className="flex items-center justify-between p-4 rounded-xl bg-surface-container-highest border border-white/5">
                  ...
                  <div className="flex gap-2 items-center">
                    <Link href={`/admin/workouts/${workout._id}/edit`} className="material-symbols-outlined text-on-surface-variant hover:text-white transition-colors">edit</Link>
                    <form action={async () => {
                      'use server';
                      await deleteWorkout(workout._id.toString());
                    }}>
        ...
        <div className="bg-surface-container-low rounded-2xl border border-white/5 p-6">
          <div className="flex items-center justify-between mb-8">
            ...
            <Link href="/admin/marketing/banners/new" className="bg-[#CCFF00] text-black px-6 py-2 rounded-full font-bold uppercase tracking-widest text-xs active:scale-95 transition-transform flex items-center gap-2">
              <span className="material-symbols-outlined text-sm">add</span> Add New
            </Link>
          </div>
          ...
          <div className="space-y-4">
            {banners.length === 0 ? (
              ...
              banners.map((ad: any) => (
                <div key={ad._id.toString()} className="flex items-center justify-between p-4 rounded-xl bg-surface-container-highest border border-white/5">
                  <div className="flex items-center gap-4">
                    <div className="w-16 h-10 rounded overflow-hidden">
                      <img src={ad.imageUrl} className="w-full h-full object-cover" />
                    </div>
                    <div>
                      <h4 className="font-bold text-white uppercase tracking-wider text-xs truncate max-w-[120px]">{ad.title}</h4>
                    </div>
                  </div>
                  <div className="flex gap-2 items-center">
                    <Link href={`/admin/marketing/banners/${ad._id}/edit`} className="material-symbols-outlined text-on-surface-variant hover:text-white transition-colors">edit</Link>
                    <form action={async () => {
                      'use server';
